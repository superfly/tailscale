...
// DebugClearNetmapCache discards stored netmap caches (if any) for profiles
// for the current user of b. It also drops any cache from the active backend
// session, if there is one.
func (b *LocalBackend) DebugClearNetmapCache(ctx context.Context) error {
	if !buildfeatures.HasCacheNetMap {
		return nil // disabled
	}
